...
### 0-10. 표지 템플릿 WYSIWYG — 사진 슬롯 직렬화 수정 (2026-09-07)

**증상**: 표지 템플릿을 적용하면 화면에는 프레임 안에 크롭돼 보이는데, 저장본·서버 렌더·
3D 미리보기·인쇄물에서는 사진이 책등을 넘어 뒷표지까지 번지고 우측 재단선을 넘었다.

**원인**: `PhotoObject` 의 박스는 **슬롯**(칸)이고 렌더러는 그 칸으로 clip 한 뒤 채운다.
그런데 Fabric 의 cover 사진은 *칸보다 큰 이미지*를 칸으로 clip 해 그리므로
**fabric bounding box ≠ 슬롯**이다. `serializeOne` 이 그 차이를 모르고
`width * scaleX`(= 이미지 박스)를 저장하면서 슬롯이 이미지 크기로 부풀었다.
실측(프레임 템플릿, 앞표지 151mm): 슬롯 131×168mm → 저장 224×168mm, leftMm 161.27 → 114.77.
4:3 사진을 131×168 슬롯에 cover 하면 높이가 구속조건이 되어 정확히 224×168 이 나온다.

**수정** (`lib/fabric/photo-slot.ts` 신설 — 슬롯 단일 소스)
- 슬롯을 `slotWidthMm/slotHeightMm/slotScaleX/slotScaleY` 태그로 보관하고
  (FABRIC_EXTRA_PROPS 에 등록 → history 라운드트립 보존),
  직렬화는 bounding box 가 아니라 이 슬롯을 기록한다. 태그 없는 legacy 객체는 기존 경로로 폴백.
- clip 을 absolutePositioned(페이지 좌표 고정) → **객체 로컬 좌표계**로 바꿨다. 그 결과
  ① 이동·확대·회전에 자동으로 따라붙어 동기화 핸들러가 아예 필요 없고,
  ② 회전 시 슬롯이 사진과 함께 도는 PDF 렌더러와 화면이 일치한다
     (이전에는 회전하면 화면과 인쇄가 달랐고, 사진을 옮기면 이미지만 크롭 창 아래로 미끄러졌다).
- 곁가지 2건도 같은 경로라 함께 고쳤다: 사진 교체 시 새 원본 크기로 슬롯 재적용
  (안 하면 교체한 사진만 슬롯을 벗어남), SelectionPanel 의 꽉채우기/맞춰넣기 토글이
  태그만 바꾸고 화면에 반영되지 않던 문제.

**검증**
- 실브라우저: 프레임 템플릿 적용 → 저장 photo `leftMm 161.18 / widthMm 131.00 / heightMm 168.00`
  (앞표지 구간 [151.18, 302.18] 안, 우측 292.18 < 302.18). 템플릿 의도와 정확히 일치.
- 적용 직후 캔버스 vs 새로고침 후 캔버스 픽셀 차이 0.78%(토스트·저장버튼 상태뿐, 사진 위치 동일).
- 서버 렌더(`POST /api/cover/preview`) PNG 도 동일 배치 — 3D 미리보기·인쇄 경로 확인.
- 회귀 가드: `lib/fabric/photo-slot.test.ts`(4건) + `serialize.test.ts` 슬롯 2건 추가.
- typecheck·lint·unit(181)·pdf 회귀(4)·a11y(25)·e2e smoke(12)·golden-flow(2) 전부 green.
...
- ~~표지 템플릿 WYSIWYG 불일치~~ → §0-10 에서 수정 완료(2026-09-07).
