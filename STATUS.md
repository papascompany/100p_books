...
DB 마이그레이션: 0001 ~ 0024 (로컬 적용 완료, 운영 DB는 0023·0024 수동 적용 필요)
...
| 항목 | 작업 | 참고 문서 |
|---|---|---|
| DB 마이그레이션 0023 적용 | Supabase SQL Editor 에서 `0023_point_ledger.sql` 실행 | `supabase/migrations/` |
| DB 마이그레이션 0024 적용 | Supabase SQL Editor 에서 `0024_user_dashboard_counts.sql` 실행 (mypage 카운트 RPC) | `supabase/migrations/` |
| Supabase 운영 DB 일괄 | `supabase db push` 또는 콘솔 SQL 실행 | - |
...
1. **Supabase 운영 DB 마이그레이션 적용** — 0023(point_ledger) + 0024(get_user_dashboard_counts) 두 건
   - 0024 미적용 시 mypage 카운트가 0 으로 fallback (페이지 자체는 정상)
