...
| **마이그레이션 0031 (보안)** | 🔴 **§0 — 먼저 적용** | **예** (포인트 무한 발행·토큰 유출 차단) |

**→ 이메일 가입 → 업로드 → 편집 → 주문 → 결제 → 인쇄검증 경로는 전부 동작한다.**
⬜ 항목은 품질/편의라 하나도 안 해도 열 수 있지만, **§0 보안 마이그레이션만은 예외다** —
오픈 전에 반드시 적용할 것.

---

## §0. 🔴 보안 마이그레이션 0031 — 오픈 전 필수 (SQL 1분)
...
`supabase/migrations/0031_tighten_public_grants.sql` 전체를 그대로 붙여넣고 Run.
`locked down: public.xxx(...)` NOTICE 가 함수 수만큼 출력되면 정상이다.

적용 후 확인 (터미널 1줄, `<ref>`/`<anon>` 은 프로젝트 값):
```bash
